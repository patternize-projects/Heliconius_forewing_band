...
rasterList_vul_sub <- patLanRGB(imageList_vul, landList_vul, RGB, transformRef = target, resampleFactor = 3,
                                colOffset = 0.2, crop = FALSE, res = 200, adjustCoords = TRUE, plot = 'stack',
                                iteration = 3)
save(rasterList_vul_sub, file = 'aligned_rasterLists/rasterList_vul_sub.rda')
...
# load if saved
load('aligned_rasterLists/rasterList_mal.rda')
load('aligned_rasterLists/rasterList_ecu.rda')
load('aligned_rasterLists/rasterList_amar.rda')
load('aligned_rasterLists/rasterList_cyt.rda')
load('aligned_rasterLists/rasterList_melFG.rda')
load('aligned_rasterLists/rasterList_melP.rda')
load('aligned_rasterLists/rasterList_mer.rda')
load('aligned_rasterLists/rasterList_agl.rda')
load('aligned_rasterLists/rasterList_nan.rda')
load('aligned_rasterLists/rasterList_ple.rda')
load('aligned_rasterLists/rasterList_ros.rda')
load('aligned_rasterLists/rasterList_vul.rda')
load('aligned_rasterLists/rasterList_the.rda')
load('aligned_rasterLists/rasterList_xen.rda')


# setMask(summedRaster_mal_M, IDList_mal ,refImage = target, 'mask/mask_mal.txt')
# setMask(summedRaster_lanRGB_ecu, IDlist_ecu, 'mask/mask_ecu.txt')
# setMask(summedRaster_lanRGB_cythera, IDlist_cyt, 'mask/mask_cyt.txt')
# setMask(summedRaster_lanRGB_melFG, IDlist_melFG, 'mask/mask_melFG.txt')
# setMask(summedRaster_lanRGB_meriana, IDlist_meri, 'mask/mask_mer.txt')
# setMask(summedRaster_lanRGB_aglaope, IDlist_agla, 'mask/mask_agl.txt')
# setMask(summedRaster_lanRGB_nanna, IDlist_na, 'mask/mask_nan.txt')
# setMask(summedRaster_lanRGB_plesseni, IDlist_ple, 'mask/mask_ple.txt')
# setMask(summedRaster_lanRGB_thelxixopeia, IDlist_thel,refImage = target, 'mask/mask_thel.txt')
# setMask(summedRaster_lanRGB_xenoclea, IDlist_xeno, refImage = target, 'mask/mask_xeno.txt')


# mask
mask_mal <- read.table("mask/mask_mal.txt", h=T)
rasterList_mal_M <-list()
for(e in 1:length(rasterList_mal)){
  ID <- names(rasterList_mal)[[e]]
  rasterList_mal_M[[ID]] <- maskOutline(rasterList_mal[[ID]], IDlist = IDList_mal, mask_mal, 
                                               refShape = 'target', imageList = imageList_mal)
}


mask_ecu <- read.table("mask/mask_ecu.txt", h=T)
rasterList_ecu_M <-list()
for(e in 1:length(rasterList_ecu)){
  ID <- names(rasterList_ecu)[[e]]
  rasterList_ecu_M[[ID]] <- maskOutline(rasterList_ecu[[ID]], IDlist = IDList_ecu, mask_ecu, 
                                               refShape = 'target', imageList = imageList_ecu)
}

mask_cyt <- read.table("mask/mask_cyt.txt", h=T)
rasterList_cyt_M <-list()
for(e in 1:length(rasterList_cyt)){
  ID <- names(rasterList_cyt)[[e]]
  rasterList_cyt_M[[ID]] <- maskOutline(rasterList_cyt[[ID]], IDlist = IDList_cyt, mask_cyt, 
                                                   refShape = 'target', imageList = imageList_cyt)
}

mask_melFG <- read.table("mask/mask_melFG.txt", h=T)
rasterList_melFG_M <-list()
for(e in 1:length(rasterList_melFG)){
  ID <- names(rasterList_melFG)[[e]]
  rasterList_melFG_M[[ID]] <- maskOutline(rasterList_melFG[[ID]], IDlist = IDList_melFG, mask_melFG, 
                                                 refShape = 'target', imageList = imageList_melFG)
}

mask_mer <- read.table("mask/mask_mer.txt", h=T)
rasterList_mer_M<-list()
for(e in 1:length(rasterList_mer)){
  ID <- names(rasterList_mer)[[e]]
  rasterList_mer_M[[ID]] <- maskOutline(rasterList_mer[[ID]], IDlist = IDList_mer, mask_mer, 
                                                   refShape = 'target', imageList = imageList_mer)
}

mask_agl <- read.table("mask/mask_agl.txt", h=T)
rasterList_agl_M<-list()
for(e in 1:length(rasterList_agl)){
  ID <- names(rasterList_agl)[[e]]
  rasterList_agl_M[[ID]] <- maskOutline(rasterList_agl[[ID]], IDlist = IDList_agl, mask_agl, 
                                                   refShape = 'target', imageList = imageList_agl)
}

mask_nan <- read.table("mask/mask_nan.txt", h=T)
rasterList_nan_M<-list()
for(e in 1:length(rasterList_nan)){
  ID <- names(rasterList_nan)[[e]]
  rasterList_nan_M[[ID]] <- maskOutline(rasterList_nan[[ID]], IDlist = IDList_nan, mask_nan, 
                                                 refShape = 'target', imageList = imageList_nan)
}

mask_ple <- read.table("mask/mask_ple.txt", h=T)
rasterList_ple_M<-list()
for(e in 1:length(rasterList_ple)){
  ID <- names(rasterList_ple)[[e]]
  rasterList_ple_M[[ID]] <- maskOutline(rasterList_ple[[ID]], IDlist = IDList_ple, mask_ple, 
                                                    refShape = 'target', imageList = imageList_ple)
}

mask_the <- read.table("mask/mask_thel.txt", h=T)
rasterList_the_M<-list()
for(e in 1:length(rasterList_the)){
  ID <- names(rasterList_the)[[e]]
  rasterList_the_M[[ID]] <- maskOutline(rasterList_the[[ID]], IDlist = IDList_the, mask_the, 
                                                       refShape = 'target', imageList = imageList_the)
}

mask_xen <- read.table("mask/mask_xeno.txt", h=T)
rasterList_xen_M<-list()
for(e in 1:length(rasterList_xen)){
  ID <- names(rasterList_xen)[[e]]
  rasterList_xen_M[[ID]] <- maskOutline(rasterList_xen[[ID]], IDlist = IDList_xen, mask_xen, 
                                                    refShape = 'target', imageList = imageList_xen)
}

# rasterList_amar_M <- rasterList_amar
# rasterList_melP_M <- rasterList_melP
# rasterList_ros_M <- rasterList_ros
# rasterList_vul_M <- rasterList_vul
# 
# save(rasterList_mal_M, file = 'aligned_rasterLists/rasterList_mal_M.rda')
# save(rasterList_ecu_M, file = 'aligned_rasterLists/rasterList_ecu_M.rda')
# save(rasterList_amar_M, file = 'aligned_rasterLists/rasterList_amar_M.rda')
# save(rasterList_cyt_M, file = 'aligned_rasterLists/rasterList_cyt_M.rda')
# save(rasterList_melFG_M, file = 'aligned_rasterLists/rasterList_melFG_M.rda')
# save(rasterList_melP_M, file = 'aligned_rasterLists/rasterList_melP_M.rda')
# save(rasterList_mer_M, file = 'aligned_rasterLists/rasterList_mer_M.rda')
# save(rasterList_agl_M, file = 'aligned_rasterLists/rasterList_agl_M.rda')
# save(rasterList_nan_M, file = 'aligned_rasterLists/rasterList_nan_M.rda')
# save(rasterList_ple_M, file = 'aligned_rasterLists/rasterList_ple_M.rda')
# save(rasterList_ros_M, file = 'aligned_rasterLists/rasterList_ros_M.rda')
# save(rasterList_vul_M, file = 'aligned_rasterLists/rasterList_vul_M.rda')
# save(rasterList_the_M, file = 'aligned_rasterLists/rasterList_the_M.rda')
# save(rasterList_xen_M, file = 'aligned_rasterLists/rasterList_xen_M.rda')
...
summedRaster_mal_M <- sumRaster(rasterList_mal_M, IDList_mal, type = 'RGB')
summedRaster_ecu_M <- sumRaster(rasterList_ecu_M, IDList_ecu, type = 'RGB')
summedRaster_amar_M <- sumRaster(rasterList_amar_M, IDList_amar, type = 'RGB')
summedRaster_cyt_M <- sumRaster(rasterList_cyt_M, IDList_cyt, type = 'RGB')
summedRaster_melFG_M <- sumRaster(rasterList_melFG_M, IDList_melFG, type = 'RGB')
summedRaster_melP_M <- sumRaster(rasterList_melP_M, IDList_melP, type = 'RGB')
summedRaster_mer_M <- sumRaster(rasterList_mer_M, IDList_mer, type = 'RGB')
summedRaster_agl_M <- sumRaster(rasterList_agl_M, IDList_agl, type = 'RGB')
summedRaster_nan_M <- sumRaster(rasterList_nan_M, IDList_nan, type = 'RGB')
summedRaster_ple_M <- sumRaster(rasterList_ple_M, IDList_ple, type = 'RGB')
summedRaster_ros_M <- sumRaster(rasterList_ros_M, IDList_ros, type = 'RGB')
summedRaster_vul_M <- sumRaster(rasterList_vul_M, IDList_vul, type = 'RGB')
summedRaster_the_M <- sumRaster(rasterList_the_M, IDList_the, type = 'RGB')
summedRaster_xen_M <- sumRaster(rasterList_xen_M, IDList_xen, type = 'RGB')


colfunc <- inferno(100)
plotHeat(summedRaster_mal_M, IDList_mal, plotCartoon = TRUE, refShape = 'target', outline = outline_BC0004, 
...
plotHeat(summedRaster_ecu_M, IDList_ecu, plotCartoon = TRUE, refShape = 'target', outline = outline_BC0004, 
         lines = lines_BC0004, landList = landList_ecu, adjustCoords = TRUE, imageList = imageList_ecu, 
         cartoonID = 'BC0004', cartoonFill = 'black', cartoonOrder = 'under', colpalette = colfunc)

plotHeat(summedRaster_amar_M, IDList_amar, plotCartoon = TRUE, refShape = 'target', outline = outline_BC0004, 
         lines = lines_BC0004, landList = landList_amar, adjustCoords = TRUE, imageList = imageList_amar, 
         cartoonID = 'BC0004', cartoonFill = 'black', cartoonOrder = 'under', colpalette = colfunc)

plotHeat(summedRaster_cyt_M, IDList_cyt, plotCartoon = TRUE, refShape = 'target', outline = outline_BC0004, 
         lines = lines_BC0004, landList = landList_cyt, adjustCoords = TRUE, imageList = imageList_cyt, 
         cartoonID = 'BC0004', cartoonFill = 'black', cartoonOrder = 'under', colpalette = colfunc)

plotHeat(summedRaster_melFG_M, IDList_melFG, plotCartoon = TRUE, refShape = 'target', outline = outline_BC0004, 
         lines = lines_BC0004, landList = landList_melFG, adjustCoords = TRUE, imageList = imageList_melFG, 
         cartoonID = 'BC0004', cartoonFill = 'black', cartoonOrder = 'under', colpalette = colfunc)

plotHeat(summedRaster_melP_M, IDList_melP, plotCartoon = TRUE, refShape = 'target', outline = outline_BC0004, 
         lines = lines_BC0004, landList = landList_melP, adjustCoords = TRUE, imageList = imageList_melP, 
         cartoonID = 'BC0004', cartoonFill = 'black', cartoonOrder = 'under', colpalette = colfunc)

plotHeat(summedRaster_mer_M, IDList_mer, plotCartoon = TRUE, refShape = 'target', outline = outline_BC0004, 
         lines = lines_BC0004, landList = landList_mer, adjustCoords = TRUE, imageList = imageList_mer, 
         cartoonID = 'BC0004', cartoonFill = 'black', cartoonOrder = 'under', colpalette = colfunc)

plotHeat(summedRaster_agl_M, IDList_agl, plotCartoon = TRUE, refShape = 'target', outline = outline_BC0004, 
         lines = lines_BC0004, landList = landList_agl, adjustCoords = TRUE, imageList = imageList_agl, 
         cartoonID = 'BC0004', cartoonFill = 'black', cartoonOrder = 'under', colpalette = colfunc)

plotHeat(summedRaster_nan_M, IDList_nan, plotCartoon = TRUE, refShape = 'target', outline = outline_BC0004, 
         lines = lines_BC0004, landList = landList_nan, adjustCoords = TRUE,  imageList = imageList_nan, 
         cartoonID = 'BC0004', cartoonFill = 'black', cartoonOrder = 'under', colpalette = colfunc)

plotHeat(summedRaster_ple_M, IDList_ple, plotCartoon = TRUE, refShape = 'target', outline = outline_BC0004, 
...
plotHeat(summedRaster_the_M, IDList_the, plotCartoon = TRUE, refShape = 'target', outline = outline_BC0004, 
         lines = lines_BC0004, landList = landList_the, adjustCoords = TRUE, imageList = imageList_the, 
         cartoonID = 'BC0004', cartoonFill = 'black', cartoonOrder = 'under', colpalette = colfunc)

plotHeat(summedRaster_xen_M, IDList_xen, plotCartoon = TRUE, refShape = 'target', outline = outline_BC0004, 
